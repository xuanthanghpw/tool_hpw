...
class SmartyApp:
    ...
    def setup_gui(self):
        ...
        self.delay_var = tk.StringVar(value="15.0")
                ...
                for index, row in df.iterrows():
                    ...
                    if delay_seconds > 0:
                        ...
                        if index % 20 == 0 and index > 0:
                            self.log("Đang tạm nghỉ 120 giây sau mỗi 20 dòng để tránh bị giới hạn...")
                            time.sleep(120)
        ...
        for attempt in range(max_retries):
            ...
            if is_rate_limited:
                self.log(
                    ...
                    f"key {key_label}, {proxy_label}). Chờ 300 giây trước khi thử lại..."
                )
                time.sleep(300)
            else:
                self.log(
                    f"  [!] Dòng {row_num}: Lỗi mạng/Timeout (lần {attempt + 1}/{max_retries}, {proxy_label}). "
                    f"Chờ 60 giây trước khi thử lại..."
                )
                time.sleep(60)

        return {
            "result": "Lỗi: Quá nhiều request hoặc lỗi mạng liên tục sau nhiều lần thử.",
            "analysis": {},
            "success": False,
        }
